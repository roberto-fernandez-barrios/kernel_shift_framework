...
- **Repeated finite-shot fidelity estimation** with 30 independent measurement replicates at four shot counts, both before and after positive-semidefinite projection, conditional on eight fixed exact Gram matrices.
...
4. Repeated finite-shot perturbations show that effective rank is measurement-sensitive and that positive-semidefinite projection can change the sign and magnitude of a fixed-run OOD deviation. These are conditional sampling experiments, not hardware simulations.
...
The reproduction commands deliberately do not rerun the expensive experiment grids; they validate and consume versioned summaries. The frozen contracts are [`docs/ANALYSIS_SPEC_V4.md`](docs/ANALYSIS_SPEC_V4.md), [`docs/EXTERNAL_VALIDATION_SPEC.md`](docs/EXTERNAL_VALIDATION_SPEC.md), and [`docs/REVIEWER_REVISION_SPEC_V6.md`](docs/REVIEWER_REVISION_SPEC_V6.md).

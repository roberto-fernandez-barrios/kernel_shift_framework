...
\abstract{Claims of quantum-kernel robustness can depend on how candidates are tuned, selected, and compared. We audit this dependence with a controlled kernel-swap benchmark under distribution shift and prospectively frozen external corroboration. In eight fixed security scenario-groups, same-test target-label selection, fixed regularization, and customary references produce apparent gains up to $+0.037$. Within one generation, moving from this bundle to train-only tuning, ID-validation selection, extended references, and matched counts changes the source-dataset-equal support-vector effect from $+0.0131$ to $-0.0047$. Under full controls it is $-0.0056$, and $-0.0009$ for Gaussian-process classification. Two of four quantum maps are separable product kernels. At matched 30-candidate budgets, entangling-ZZ and product-map support-vector effects are $-0.0093$ and $-0.0055$. Across three fixed external tasks, the controlled support-vector effect is $-0.0119$. Neither stronger controls nor restriction to entangling maps reveals consistent evidence of a positive quantum-family effect in these low-qubit fixed cases.}
...
\subsection{Within-v4 sensitivities separate the evaluation bundle}
\label{sec:res_v8_factorial}

The within-v4 factorial removes the generation boundary from the historical specification map. In the q1000 SVC stratum, the fixed-$C$, same-test-oracle, customary-reference, native-budget endpoint is $+0.0131$, whereas the train-CV, ID-validation, extended-reference, equal-count endpoint is $-0.0047$, a descriptive contraction of $0.0178$. The 16 three-source-dataset-equal endpoints range from $-0.0084$ to $+0.0131$ (Figure~\ref{fig:v8_revision}; Supplementary Table~10). Thus the sign change is present within one result generation and does not require the legacy-to-v4 boundary.
...
One case--replicate samples 1,624,250 input-pair fidelities: 499,500 train-triangle entries, 500,000 combined ID-holdout--train entries, 500,000 OOD--train entries, and 124,750 OOD-triangle entries. The ID-validation and ID-test roles partition the 500-row holdout and therefore need not each contain exactly 250 rows. This corresponds to 207,904,000 shots at 128 shots per fidelity and 13,305,856,000 at 8,192. Across eight cases, there are 12,994,000 case-conditioned parameterizations; repeating them 30 times gives 389,820,000 fidelity-estimation instances per shot level. Estimating all eight fixed configurations by circuits at all four levels projects 4.24 trillion shots (Supplementary Table~14). Four configurations are separable-product maps, however, so direct classical factorization can bypass their circuit sampling; the corresponding projection for the four entangling cases alone is 2.12 trillion shots. The three PSD conditions reuse the same sampled measurements. These are algebraic projections before device routing, calibration, mitigation, and repeated job overhead, not executed hardware costs or minimum implementation costs.

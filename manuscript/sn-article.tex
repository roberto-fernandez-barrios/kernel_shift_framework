...
Circuit decomposition changes the interpretation of the aggregate family. The two ZZFeatureMap variants contain all-to-all two-qubit Pauli interactions, whereas the Pauli-X/Z and ZFeatureMap variants contain only one-qubit Pauli strings. For the latter pair, the prepared state and its fidelity kernel factorize over coordinates (Methods). Thus two of the four map families are quantum-generated implementations of product kernels and cannot supply evidence that entanglement is responsible for their behaviour.
...
\subsection{Geometry tracks robustness only partially}
...
One case--replicate samples 1,624,250 distinct input-pair fidelities: 499,500 train-triangle entries, 250,000 validation--train, 250,000 ID-test--train, 500,000 OOD--train, and 124,750 OOD-triangle entries. This corresponds to 207,904,000 shots at 128 shots per fidelity and 13,305,856,000 at 8,192. Across eight cases and 30 replicates, one shot level entails 389,820,000 distinct circuit parameterizations; the complete four-level sensitivity projects 4.24 trillion shots. The three PSD conditions reuse the same sampled measurements. These are algebraic resource projections before device routing, calibration, mitigation, and repeated job overhead, not executed hardware costs.
...
From a practical perspective, no consistent fidelity-kernel advantage is observed over the tuned classical family, while overlap estimation introduces circuit and shot costs absent from direct classical-kernel evaluation. A tuned, shortened-length-scale Laplacian is therefore an important inexpensive reference. This is not a universal recommendation: any proposed quantum construction should be tested in its intended shift regime under the same controls and with its resource budget reported.
...
The appropriate interpretation is conservative. We do not claim universal superiority, equivalence, complexity-theoretic advantage, hardware relevance, or a refutation of quantum kernels. We show that an optimistic fixed-case result changes under deployable controls; distinguish the behaviour and resources of entangling and product maps; and find that the measured geometry descriptors are shared, post-hoc correlates rather than provenance-specific explanations. This is consistent with calls for controlled QML benchmarking \cite{bowles2024betterthanclassical,schnabel2025scrutiny,kakavand2026benchmarking}.
...
\]
so the constructor argument \texttt{entanglement="full"} stored for the Pauli-X/Z map has no effect without a multiqubit Pauli string. We call these the entangling-ZZ and separable-product strata. Each map uses angle scales $\{0.5,1,2\}$ and all five dimensions, giving 60 candidates (30 per stratum) at 4--12 simulated qubits. Classical bandwidth and quantum angle-scale freedom both control locality \cite{shaydulin2022bandwidth,canatar2023bandwidth}; no circuit is trained.

\subsection{Circuit and sampling-resource audit}

Circuit counts use Qiskit 2.3.1 and the logical basis \{\texttt{rz}, \texttt{sx}, \texttt{x}, \texttt{cx}\} at transpiler optimization level 0, without backend layout or routing. We report each feature-map template and the compute--uncompute fidelity template $U(x)U^\dagger(x')$ after assigning distinct numeric inputs. Counts are compiler- and basis-conditional logical resources, not device schedules.

For the repeated q1000 finite-shot sensitivity, each case and measurement replicate estimates the off-diagonal training triangle, the ID-validation-, ID-test-, and OOD-test-to-training rectangles, and the off-diagonal OOD square triangle; diagonals are fixed to one. Thus one kernel configuration requires
\[
\binom{1000}{2}+250(1000)+250(1000)+500(1000)+\binom{500}{2}
=1{,}624{,}250
\]
...
\]
\cite{cristianini2001kta,cortes2012centeredalignment}. KTA is computed separately on training, ID, and OOD square blocks, alongside off-diagonal and cross-split similarities. OOD KTA reuses the same OOD labels as accuracy and is therefore a post hoc diagnostic, not a label-independent mechanism. A legacy EMBER-only implementation of the geometric difference of Huang et al.\ remains in the archive, but it is not part of the complete v4 estimand and no manuscript conclusion relies on it \cite{huang2021powerofdata}.
...
Experiments use Python 3.11/3.12, NumPy 2.3.5, pandas 2.2.3, scikit-learn 1.8.0, Matplotlib 3.11.0, and Qiskit 2.3.1. The Windows-specific Conda snapshot and cross-platform package specifications serve different archival purposes; neither is an exact machine image of every full-scale run. Code, manifests, automated author-provided audits, tests, aggregated results, and generators are available at \url{https://github.com/roberto-fernandez-barrios/kernel_shift_framework}. The immutable v0.8.0 submission DOI will identify the exact code, results, and PDFs; the concept DOI resolves all versions.

...
\section*{Supplementary Results: Prospective Gate-2 replication}
\label{supp:v10_gate2}

The Gate-2 protocol was frozen before acquiring any of its three tasks,
...
\begin{table}[H]
\centering
\caption{Technical disposition of every frozen prospective Gate-2 task.
...
The frozen outcome summaries are the medians over five seeds in each
...
condition for strong prospective transfer. The reported category remains
\texttt{strong prospective transfer---technically limited two-task
replication} because only two tasks completed the grid.
...
this experiment validates transfer of Gate~2 on the eligible fixed tasks but
does not independently replicate the retrospective entangling-ZZ result.

\section*{Supplementary Results: Cross-generation sensitivity map}
...
analysis specifications across two result generations; it is not a factorial
decomposition of their individual choices.

\begin{table}[ht]
\centering
\caption{Prespecified cross-generation definitions and source-dataset-equal
...
Table~\ref{supp:tab_intervals} compares the primary pointwise conditional
...
Table~\ref{supp:tab_quantum_strata} reports every fixed-case SVC effect after
...
shown in Table~\ref{supp:tab_quantum_winners} and provided in
\texttt{quantum\_winner\_composition.csv}.
...
\section*{Supplementary Results: Within-v4 factorial}
\label{supp:factorial}

The q1000 factorial holds the v4 data generation, split roles, kernel
inventory, and preprocessing fixed while crossing regularization, selector,
classical reference, and budget mode. Table~\ref{supp:tab_factorial} reports
...
\begin{table}[htbp]
\centering
\caption{Within-v4 q1000 SVC factorial. Entries are three-source-dataset-equal
...
Table~\ref{supp:tab_factorial_contrasts} summarizes paired axis changes and
...
affect the two kernel families differently. Table~\ref{supp:tab_shortcut}
...
comparably (Figure~\ref{supp:fig_uncertainty}). Together with the controlled
...
\section*{Supplementary Results: Geometry displays moved from the main text}
...
within-v4 factorial remain under \texttt{results/v8/}; prior inference,
...
and the immutable v1.1.0 archive is
\url{https://doi.org/10.5281/zenodo.21750725}; the all-version concept DOI is
\url{https://doi.org/10.5281/zenodo.19147649}. The exact v0.8.0 predecessor is
identified by \url{https://doi.org/10.5281/zenodo.21717074}. The v1.0
prospective inputs and outputs are under
...
are included in the v1.1.0 archive together with the bounded-loss tests,

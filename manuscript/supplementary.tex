...
\hypersetup{
  pdftitle={Supplementary Information: Evaluation Choices Determine Apparent Quantum Kernel Robustness under Distribution Shift},
...
GPC. Values are descriptive fixed-case summaries.}
...
Figure~\ref{supp:fig_shots} displays all 1,920 finite-shot evaluations:
eight fixed exact Gram matrices, four shot counts, 30 measurement replicates,
and pre-/post-PSD conditions. The bands make the conditional Monte Carlo
spread visible rather than treating diversity across data configurations as
measurement replication. PSD projection often narrows or shifts the
distribution but does not impose a uniform direction. Exact-statevector OOD
accuracy is approached unevenly because each sampled training matrix
reselects a discrete SVC $C$.
...
Orange squares use the sampled indefinite matrix; blue circles use its PSD
projection. The model includes fidelity sampling only, not device noise.}
\label{supp:fig_shots}
\end{figure}

Across the eight group medians, the perturbed/exact effective-rank ratio
declines from 1.93 at 128 shots to 1.13 at 8,192 shots. The across-group median
OOD-alignment change remains within 0.0007 of zero. Separate train and
OOD-square Frobenius changes due to sampling and projection, negative-spectrum
...
The canonical v0.6.0 release gates are
\begin{verbatim}
